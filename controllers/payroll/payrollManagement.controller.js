...
const calculateSalaryComponents = (basicSalary,payParam,employeeType) => {
  ...
  let esi = 0
   ...
   if (employeeType === "PERMANENT" || employeeType === "CONTRACT") {
    epf = basic <= Number.parseFloat(payParam.epfSalaryLimit) ? basic * (Number.parseFloat(payParam.epfRate) / 100) : 0
  ...
  const professionalTax = grossSalary * (Number.parseFloat(payParam.professionalTaxRate) / 100)  
...
const generateSalary = asyncHandler(async(req,res) => {
  ...
  const employeeFilter = {
   ...
   if (departmentIds && departmentIds.length > 0) {
    employeeFilter.departmentId = { in: departmentIds.map((id) => Number.parseInt(id)) }
  } 
    ...
    where: {
      ...
      ...(departmentIds &&
        departmentIds.length > 0 && {
          employee: { departmentId: { in: departmentIds.map((id) => Number.parseInt(id)) } },
        }),
      ...(employeeIds &&
    ...
    for (const employee of employees) {
      try {
        ...
        if (!employee.salary || employee.salary <= 0) {
          errors.push(`Employee ${employee.employeeCode}: No valid salary found`)
          continue
        }
        ...
        if (!payParam) {
          errors.push(`Employee ${employee.employeeCode}: No pay parameters found for type ${employee.type}`)
          continue
        }

        // Calculate salary components based on employee type
        const salaryComponents = calculateSalaryComponents(employee.salary, payParam, employee.type)
  ...
  if (generatedSalaries.salaryRecords.length === 0) {
    throw new ApiError(400, "No salaries were generated. Check pay parameters configuration.")
  }
  ...
  const response = {
    ...
    ...(generatedSalaries.errors.length > 0 && { errors: generatedSalaries.errors }),
  }
...
const getEmployeeSalaries = asyncHandler(async(req,res)=> {
  ...
  const [salaries, totalCount] = await Promise.all([
    prisma.payMaster.findMany({
      ...
      skip,
      take,
      orderBy: [{ year: "desc" }, { month: "desc" }, { employee: { name: sortOrder } }],
      include: {
        employee: {
          select: {
            employeeCode: true,
  ...
  if (employeeType) {
    filteredSalaries = filteredSalaries.filter((salary) => salary.employee.type === employeeType)
  }
  ...
  if (designationId) {
    filteredSalaries = filteredSalaries.filter(
      (salary) => salary.employee.designationId === Number.parseInt(designationId),
    )
  }
...
const updateSalary = asyncHandler(async(req,res)=> {
  ...
  if (!hasPermission(currentUser.role, userType, "payroll:update")) {
   ...
   const newOtherAll =
    otherAll !== undefined ? Number.parseFloat(otherAll) : Number.parseFloat(existingPayMaster.otherAll)
  ...
  const newBasicSalary =
    basicSalary !== undefined ? Number.parseFloat(basicSalary) : Number.parseFloat(existingPayMaster.basicSalary)

  if (newOtherAll < 0 || newOtherDeductions < 0) {
    throw new ApiError(400, "Other allowances and deductions cannot be negative")
  }
  ...
  let updatedComponents = {}  
   ...
   if (basicSalary !== undefined) {
    ...
    if (payParam) {
      const recalculated = calculateSalaryComponents(newBasicSalary, payParam, existingPayMaster.employee.type)
      updatedComponents = {
  ...
  const baseDeductions =
    Object.keys(updatedComponents).length > 0
      ? updatedComponents.epf + updatedComponents.esi + updatedComponents.tds + updatedComponents.professionalTax
  ...
  return res.status(200).json(new ApiResponse(200, updatedPayMaster, "Salary updated successfully"))
})
