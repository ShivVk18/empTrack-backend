...
const getPayrollTrends = asyncHandler(async(req,res)=> {
    ...
    baseWhereClause.employeeId = { in: departmentEmployees.map((emp) => emp.id) }
  }
  ...
  for(let i=0;i<monthToFetch;i++) {
     ...
     const monthlyData = await prisma.payMaster.aggregate( {
        ...
        esi: true,
        tds: true,
         } ,_count:{id:true} , _avg:{netSalary:true}
     })
  ...
  const trendsWithGrowth = trends.map((trend,index) =>  {
      ...
      const growth  = {
        ...
        ? (((trend.totalGross - prevTrend.totalGross) / prevTrend.totalGross) * 100).toFixed(2)
        : 0,
      netSalary: prevTrend.totalNet
        ? (((trend.totalNet - prevTrend.totalNet) / prevTrend.totalNet) * 100).toFixed(2)
        : 0,
  ...
  return res.status(200).json(new ApiResponse(200,trendsWithGrowth, "Payroll trends fetched successfully"))
...
const getPayrollCostAnalysis = asyncHandler(async(req,res) => {
  ...
  const targetMonth = month ? Number.parseInt(month) : new Date().getMonth() + 1
  const targetYear = year ? Number.parseInt(year) : new Date().getFullYear() 
  ...
  const costPercentages = {
    allowances: {
      basic: ((componentTotals.basicSalary / componentTotals.grossSalary) * 100).toFixed(2),
      ...
      hra: ((componentTotals.hra / componentTotals.grossSalary) * 100).toFixed(2),
      medical: ((componentTotals.medicalAll / componentTotals.grossSalary) * 100).toFixed(2),
    },
    deductions: {
      epf: ((componentTotals.epf / componentTotals.grossSalary) * 100).toFixed(2),
      esi: ((componentTotals.esi / componentTotals.grossSalary) * 100).toFixed(2),
      tds: ((componentTotals.tds / componentTotals.grossSalary) * 100).toFixed(2),
      professionalTax: ((componentTotals.professionalTax / componentTotals.grossSalary) * 100).toFixed(2),
    },
  }  
  ...
  return res.status(200).json(
    new ApiResponse(
      200,
      {
        ...
        averageCostPerEmployee: componentTotals.grossSalary / costAnalysis.length,
      },
...
const getComplianceReport = asyncHandler(async(req,res) => {
  ...
  if (!hasPermission(currentUser.role, userType, "payroll:read")) {
    throw new ApiError(403, "Insufficient permissions to view compliance report")
  }
  ...
  const complianceIssues = []  
   ...
   payrollData.forEach((record) => {
    ...
    if (record.grossSalary <= 25000 && (employee.type === "PERMANENT" || employee.type === "CONTRACT")) {
  ...
  complianceStats.complianceScore = (
    ((complianceStats.epfCompliant + complianceStats.esiCompliant + complianceStats.minimumWageCompliant) /
